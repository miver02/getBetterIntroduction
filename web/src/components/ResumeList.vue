...
<template>
  <div class="resume-list">
    <p v-if="resumes.length === 0" style="text-align: center; color: #666; padding: 2rem;">
      请在左侧输入职位名称并上传简历文件开始筛选
    </p>
    
    <table v-else style="width: 100%; border-collapse: collapse; margin-bottom: 2rem;">
      <thead>
        <tr style="background-color: #f5f7fa; text-align: left;">
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0;">排名</th>
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0;">姓名</th>
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0;">年龄</th>
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0;">工龄</th>
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0; text-align: center;">契合度</th>
          <th style="padding: 12px 15px; border-bottom: 1px solid #e0e0e0; text-align: center;">详情</th>
        </tr>
      </thead>
      ...
      <tbody>
        <tr v-for="(resume, index) in resumes" :key="index">
          ...
          <td style="padding: 12px 15px; border-bottom: 1px solid #f0f0f0; text-align: center;">
            <div class="rating">
              <span 
                ...
                :key="star" 
                class="star"
                :style="{ color: star <= getStarCount(index) ? '#ffce54' : '#ddd' }"
              >
          ...
          <td style="padding: 12px 15px; border-bottom: 1px solid #f0f0f0; text-align: center;">
            <button class="view-button" @click="$emit('view-detail', resume)">查看详情</button>
          </td>
        </tr>
      </tbody>
    </table>
